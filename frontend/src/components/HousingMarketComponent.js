...
const HousingMarketComponent = () => {
  ...
  useEffect(() => {
    const fetchStates = async () => {
      try {
        const response = await axios.get('http://127.0.0.1:5000/api/housing_data');
  ...
  // form submission to fetch line graph
  const handleGenerateGraph = async (e) => {
    ...
    try {
      const response = await axios.post(
        'http://127.0.0.1:5000/api/housing_market_graph',
        { state: selectedState, county }, // Send abbreviation
        { responseType: 'blob' } // Handle binary data (image)
      );

      const url = URL.createObjectURL(new Blob([response.data]));
      setGraphUrl(url); // graph image URL
    } catch (err) {
      console.error('Failed to fetch housing market graph:', err.response?.data || err.message);
      setError(
    ...
    <div>
      <section className="py-12 bg-green-700">
        <div className="container mx-auto text-center">
          <h2 className="text-3xl font-bold mb-8 text-white">Housing Market Over Time</h2>
          <p className="text-lg text-white max-w-3xl mx-auto mb-6">
           Based on the{' '}
            ...
            , this metric reflects the typical home value over time, encompassing single-family homes and condos. Compare the trends in typical home values for a specific area to the national average to understand how home prices have evolved locally and nationally.
          </p>
...
export default HousingMarketComponent;
